...
#Latent_space
        ...
        it_d += 1
        writer.add_scalar('loss_small_mse', loss_small_mse.detach().cpu().numpy(), global_step=it_d)
        writer.add_scalar('loss_small_ssim',  loss_small_ssim.detach().cpu().numpy(), global_step=it_d)
        writer.add_scalar('loss_small_lpips', loss_small_lpips.detach().cpu().numpy(), global_step=it_d)
        writer.add_scalar('loss_medium_mse',  loss_medium_mse.detach().cpu().numpy(), global_step=it_d)
        writer.add_scalar('loss_medium_ssim',  loss_medium_ssim.detach().cpu().numpy(), global_step=it_d)
        writer.add_scalar('loss_medium_lpips',  loss_medium_lpips.detach().cpu().numpy(), global_step=it_d)
        writer.add_scalar('loss_img_mse', loss_img_mse.detach().cpu().numpy(), global_step=it_d)
        writer.add_scalar('loss_img_ssim',  loss_img_ssim.detach().cpu().numpy(), global_step=it_d)
        writer.add_scalar('loss_img_lpips',  loss_img_lpips.detach().cpu().numpy(), global_step=it_d)
        writer.add_scalar('loss_w',  loss_w.detach().cpu().numpy(), global_step=it_d)
        writer.add_scalar('loss_w_m',  loss_w_m.detach().cpu().numpy(), global_step=it_d)
        writer.add_scalar('loss_w_s',  loss_w_s.detach().cpu().numpy(), global_step=it_d)
        writer.add_scalar('loss_kl_w',  loss_kl_w.detach().cpu().numpy(), global_step=it_d)
        writer.add_scalar('loss_cosine_w',  loss_cosine_w.detach().cpu().numpy(), global_step=it_d)
        writer.add_scalar('loss_c', loss_c.detach().cpu().numpy(), global_step=it_d)
        writer.add_scalar('loss_c_m/',  loss_c_m.detach().cpu().numpy(), global_step=it_d)
        writer.add_scalar('loss_c_s',  loss_c_s.detach().cpu().numpy(), global_step=it_d)
        writer.add_scalar('loss_kl_c',  loss_kl_c.detach().cpu().numpy(), global_step=it_d)
        writer.add_scalar('loss_cosine_c', loss_cosine_c.detach().cpu().numpy(), global_step=it_d)
