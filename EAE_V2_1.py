...
def space_loss(imgs1,imgs2,image_space=True,lpips=None):
    ...
    loss_kl_imgs = torch.where(torch.isnan(loss_kl_imgs),torch.full_like(loss_kl_imgs,0), loss_kl_imgs)
    loss_kl_imgs = torch.where(torch.isinf(loss_kl_imgs),torch.full_like(loss_kl_imgs,1), loss_kl_imgs)
...
def train(avg_tensor = None, coefs=0, tensor_writer=None):
    ...
    E.load_state_dict(torch.load('_yucheng/myStyle/myStyle-v1/EAE-car-cat/result/D2E_CAT_v2)/models/E_model_ep60000.pth'))
    Gs.cuda()
    #Gm.cuda()
    E.cuda()
    const_ = Gs.const
...
#Latent Space
    ...
    ##--C
        ...
        writer.add_scalar('loss_imgs_ssim', loss_imgs_info[1], global_step=it_d)
        ...
        writer.add_scalar('loss_w_ssim', loss_w_info[1], global_step=it_d)
        writer.add_scalar('loss_w_cosine', loss_w_info[2], global_step=it_d)
        writer.add_scalar('loss_w_kl', loss_w_info[3], global_step=it_d)
        ...
        writer.add_scalar('loss_c_ssim', loss_c_info[1], global_step=it_d)
        writer.add_scalar('loss_c_cosine', loss_c_info[2], global_step=it_d)
        writer.add_scalar('loss_c_kl', loss_c_info[3], global_step=it_d)
        ...
        writer.add_scalars('Image_Space_SSIM', {'loss_mask_ssim':loss_mask_info[1],'loss_grad_ssim':loss_grad_info[1],'loss_img_ssim':loss_imgs_info[1]}, global_step=it_d)
        ...
        writer.add_scalars('Image_Space_KL', {'loss_mask_kl':loss_mask_info[3],'loss_grad_cosine':loss_grad_info[3],'loss_imgs_cosine':loss_imgs_info[3]}, global_step=it_d)
        ...
        writer.add_scalars('Latent Space W', {'loss_w_mse':loss_w_info[0][0],'loss_w_mse_mean':loss_w_info[0][1],'loss_w_mse_std':loss_w_info[0][2],'loss_w_ssim':loss_w_info[1],'loss_w_cosine':loss_w_info[2],'loss_w_kl':loss_w_info[3]}, global_step=it_d)
        writer.add_scalars('Latent Space C', {'loss_c_mse':loss_c_info[0][0],'loss_c_mse_mean':loss_w_info[0][1],'loss_c_mse_std':loss_w_info[0][2],'loss_c_ssim':loss_w_info[1],'loss_c_cosine':loss_w_info[2],'loss_c_kl':loss_w_info[3]}, global_step=it_d)
        ...
        if epoch % 100 == 0:
            ...
            with open(resultPath+'/Loss.txt', 'a+') as f:
                ...
                print('[loss_imgs_mse[img,img_mean,img_std], loss_imgs_ssim, loss_imgs_cosine, loss_kl_imgs, loss_imgs_lpips]',file=f)
